...
* Ce nouvel AZERTY se trouve dans le paquet « Un-nouvel-AZERTY-Windows-6.2.1 », dans le dossier « installer », dans les sous-dossiers « kbfrFRs1-6.2.1.0 » et « kbfrFRs2-6.2.1.0 », chacun avec ses 4 pilotes de disposition, ses 3 installateurs et son setup.exe.

* Dans le paquet « Un-nouvel-AZERTY-Windows-variantes-6.2.1 », le dossier « installer » contient les autres dispositions de la gamme disponible sous Windows :

  * Le dossier « kbbrFRs1-6.2.1.0 » contient la variante AZERTY pour la langue bretonne, avec le "cʼh" sur le ⟦1⟧, et le "ñ" sur le ⟦8⟧ à la place du tiret bas (qui est aussi sur AltGr + ⟦J⟧).

  * Le dossier « kbfrPFs1-6.2.1.0 » contient la variante AZERTY pour la Polynésie française, avec le ʻeta sur le ⟦1⟧, et une touche morte tārava sur le ⟦8⟧.

  * Le dossier « kbfrAFs1-6.2.1.0 » contient la variante AZERTY pour l’Afrique francophone, pour le bambara en particulier, avec l’epsilon latin "ɛ" sur le ⟦1⟧, et le o ouvert "ɔ" sur le ⟦8⟧.

  * Le dossier « kbfrFRr1-6.2.1.0 » contient la variante redisposée de base de ce nouvel AZERTY, disposée en QZJFGY de manière plus ergonomique tout en restant assez proche de l’AZERTY, afin que les exigences d’ergonomie n’obligent pas à renoncer aux fonctionnalités de ce nouvel AZERTY.
...
5. Télécharger le code source de ce nouvel AZERTY depuis [la page d’accueil](https://github.com/dispoclavier/nouvel-azerty/archive/refs/heads/main.zip) ou [la page d’une version](https://github.com/dispoclavier/nouvel-azerty/archive/refs/tags/6.2.1.zip).
